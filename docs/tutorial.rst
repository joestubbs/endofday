...
``storage_system`` within the directory ``home_dir``. If ``storage_system`` or ``home_dir`` is not supplied, endofday
will attempt to use a sensible default for the tenant of Agave. Please note that some tenants do not provide default
...
The endofday engine recognizes data and applications registered with Agave. We look at each individually.


Specifying Global Inputs As Agave URIs
======================================

One or more global inputs can be specified as Agave URIs of the form ``agave://my.storage.system.id//path/to/file`` as well as any publicly available URI
...
As part of the task dependency analysis, endofday will determine if a global input is used by a local task. If so, it
will automatically create a download task to retrieve the resource and insert it into the proper place in the workflow.
...
in the Agave catalog. The endofday engine executes these applications by submitting a job to the Agave jobs service.
Agave in turn executes the application on the execution system defined in the application definition, and endofday
monitors the job status until the application completes. Outputs from an Agave application can be used as inputs to
another task just like other outputs. Note that if an Agave application output is needed as the input for a task
...
The yaml syntax used to define an Agave application process is similar to Docker container processes, with a few
exceptions. We illustrate with an example from the Validate workflow.
...
The above YAML snippet defines a processes section with a single process. Specifying ``execution: agave_app``
indicates to the endofday container that this process is an Agave app. Instead of specifying ``image`` as we did
for a Docker container, ``app_id: FaST-LMM-2.07`` is specified providing the Agave application id, in this case
``FaST-LMM-2.07``. Note that the username specified in the ``endofday.conf`` must have permission in Agave to
execute the application.

The ``inputs`` stanza differs slightly from that of a Docker container process to accommodate Agave's application
definition. The ``inputs`` section is a YAML collection with an entry for each defined input for the application;
...
a YAML collection whose keys are the id's of the parameters and the values are the values to be supplied.
...
supplying the Agave application output id instead of a relative path; however, since defining outputs is option for an
Agave application, this approach will only be valid for some applications. The ``<identifier>`` can be any valid
string and is used to reference the output in other sections of the workflow definition.
...
Then endofday binary itself is registered as an Agave application for participating tenants, and as such, users can
...
.. _data: http://preview.agaveapi.co/documentation/tutorials/data-management-tutorial/#importing-data
.. _documentation: http://docs.docker.com/installation/
.. _beginners_guides: http://preview.agaveapi.co/documentation/beginners-guides/
