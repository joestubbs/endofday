...
3. Build the latest eod container and run tests using the alias
    $ docker run --rm -it --entrypoint=py.test -v /:/host -v $(pwd):/staging -e RUNNING_IN_DOCKER=true -e STAGING_DIR=$(pwd) eod /tests/test_tasks.py
...
def test_basic_task_file_attrs(task_file):
    ...
    # assert task_file.glob_ins == '- inp
    # ut <- /home/jstubbs/github-repos/endofday/examples/input.txt - loc_in <- loc_in.txt'
